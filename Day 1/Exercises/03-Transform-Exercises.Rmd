...
* _Boys_ named Sue  
* Names that were given to exactly 5 or 6 children in 1880  
...
Remember that R generally sorts from smallest to largest, so you may need to use `desc()` to reverse the order.
...
select(ess_mini, height:happy) # Select from:to
select(ess_mini, -c(main_activity, working_hours)) # Unselect columns
...
select(billboard, artist, track, num_range("wk", 10:15)) # Select weeks 10 to 15
```
...
1. Filters babynames to just the boys that were born in 2017, *then...*  
...
Note that you can also use |> to pipe the results of one function into the next. This is native to R, so does not require any additional packages - but %>% is still more common in the tidyverse.
...
* Make tables of summaries with `summarise()` (NB: tidyverse support British and US spelling.) 
...
babynames %>% 
    ...
    group_by(name, sex) %>% 
    summarise(median_rank = median(rank), .groups = "drop") %>% 
    group_by(sex) %>% 
    arrange(median_rank) %>% 
    slice_min(median_rank, n = 10) %>% # To select the top 10 (per sex)
    ggplot() +
    geom_col(mapping = aes(x = median_rank, y = fct_reorder(name, -median_rank))) +
    facet_wrap(vars(sex), scales = "free_y") +
    theme_bw() +
    labs(x = "Median Rank", y = "", title = "Top 10 Names by Median Rank")
