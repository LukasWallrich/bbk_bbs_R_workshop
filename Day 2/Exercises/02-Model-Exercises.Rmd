...
* Have a look at the code above to see how "ess_mini.RDS" is loaded from the "data" folder 
...
```{r}
ess_mini_UK <- ess_mini %>% filter(country == "GB")
ess_mini_UK <-  filter(ess_mini, country == "GB")

ggplot(ess_mini_UK, aes(x = gender, y= working_hours)) + geom_boxplot()

ess_mini_UK %>%
  group_by(gender) %>%
  summarise(mean(working_hours, na.rm = TRUE))
...
```{r}
t.test(working_hours ~gender, ess_mini_UK)

pairwise_t_tests(data = ess_mini, happy ~ country, p.adjust.method = "bonferroni") %>% dplyr::select(var_2, var_1, apa)
```
...
* Run the lm() model and pipe it into summary(). What parts can you interpret?
